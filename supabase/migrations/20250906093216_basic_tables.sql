-- Базовые таблицы для AI Researcher Console
...
CREATE TABLE IF NOT EXISTS public.session_domains (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_session_domains_session_id ON public.session_domains(session_id);
...
DROP POLICY IF EXISTS "Users can create sessions" ON public.analysis_sessions;
CREATE POLICY "Users can create sessions" ON public.analysis_sessions
    FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all sessions" ON public.analysis_sessions;
...
DROP POLICY IF EXISTS "Users can view own session domains" ON public.session_domains;
...
DROP POLICY IF EXISTS "Users can create session domains" ON public.session_domains;
